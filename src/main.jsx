...
function QuizHub({ track, progress, setProgress }) {
  const { quizQuestions: questions, exam } = track
  const minutes = exam?.practiceMinutes || estimatedQuizMinutes(questions.length)
  const [mode, setMode] = useState(null)
  if (mode) return <Quiz questions={questions} mode={mode} finish={(result) => { setProgress(p => ({ ...p, [mode]: result })); setMode(null) }} />
  return <div className="page narrow"><div className="page-title"><span>Measure, learn, measure again</span><h1>{exam ? 'Certification assessment' : 'Knowledge check'}</h1><p>This {questions.length}-question assessment samples every module. Use the same concept pool before and after studying; questions and answer positions are randomized for every attempt.</p></div>{exam ? <section className="exam-facts"><div><span>Version</span><strong>{exam.version}</strong></div><div><span>Questions</span><strong>{exam.questions}</strong></div><div><span>Time</span><strong>{exam.timeDisplay || `${exam.minutes} min`}</strong></div><div><span>Official pass mark</span><strong>{exam.passDisplay || `${exam.passPercent}%`}</strong></div><div><span>Format</span><strong>{exam.format}</strong></div></section> : null}<div className="quiz-options"><QuizOption title="Baseline quiz" result={progress.baseline} copy={`Take this before studying. Allow about ${minutes} minutes; guessing is useful because it shows what to prioritize.`} onClick={() => setMode('baseline')} /><QuizOption title="Final quiz" result={progress.final} copy={`Take this after all modules. Allow about ${minutes} minutes and compare your score and confidence.`} onClick={() => setMode('final')} /></div><ScoreMeaning exam={exam} />{track.disclaimer ? <p className="learning-disclaimer">{track.disclaimer}</p> : null}</div>
}
