  ...
  return <div className="page overview-page">
    ...
    <section className="dashboard-grid">
      <div className="left-flow">
        ...
        <button className="quiz-callout" onClick={() => navigate('quiz')}><ClipboardCheck /><span><strong>{progress.baseline ? 'Review your quiz results' : 'Start baseline quiz'}</strong><small>{role.quizQuestions.length} questions · about {estimatedQuizMinutes(role.quizQuestions.length)} min · no pressure</small></span><ChevronRight /></button>
...
function QuizHub({ questions, progress, setProgress }) { const [mode, setMode] = useState(null); if (mode) return <Quiz questions={questions} mode={mode} finish={(result) => { setProgress(p => ({ ...p, [mode]: result })); setMode(null) }} />; return <div className="page narrow"><div className="page-title"><span>Measure, learn, measure again</span><h1>Knowledge check</h1><p>This {questions.length}-question assessment samples every module. Use the same question set before and after studying: your baseline identifies gaps, and your final score shows growth.</p></div><div className="quiz-options"><QuizOption title="Baseline quiz" result={progress.baseline} copy={`Take this before studying. Allow about ${estimatedQuizMinutes(questions.length)} minutes; guessing is useful because it shows what to prioritize.`} onClick={() => setMode('baseline')} /><QuizOption title="Final quiz" result={progress.final} copy={`Take this after all modules. Allow about ${estimatedQuizMinutes(questions.length)} minutes and compare your score and confidence.`} onClick={() => setMode('final')} /></div><ScoreMeaning /></div> }
...
function ScoreMeaning() { return <section className="score-meaning"><h2>How to read your score</h2><div><p><b>&lt;40%</b> Start at the beginning. Focus on vocabulary, mental models, and safe sequencing.</p><p><b>40–69%</b> Your foundation is growing. Revisit missed topics and practice the workflows aloud.</p><p><b>70–84%</b> Strong working knowledge. Tighten your troubleshooting, risk, and communication answers.</p><p><b>85–100%</b> Ready for scenario practice. Explain your evidence, tradeoffs, validation, and follow-up.</p></div></section> }
